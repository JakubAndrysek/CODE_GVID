#cs#cs(module guess mzscheme
     ...
     (define TITLE "Hadani cisel")

     (define WELCOME "Vitejte do hry Hadani cisel") 
     ...
     (define STOPS (list 'Trefa 'trefa 'TREFA))
...
;                                     
     ...
     (define (init-game number-of-digits convert check-guess)
       (local ((define GUESS number-of-digits)
               ...
               (define (call-back x y)
                 (let ((response (check-guess (convert guesses) the-number)))
                   ...
                   (when (memq response STOPS)
                     ...
                     (make-object button% "Nova hra?" message-panel 
                       (lambda (x y) (new-game)))
                     (make-object button% "Zavrit?" message-panel
         ...
         (new-game)
         (make-object button% "Co ty na to?" guess-panel call-back)
         (send frame show #t)
         #t))
