...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(recur  loop  ([f  (max-floor)]  [l  null])
     ...
     (define  list-of-all-calls
       (lambda  ()
	(recur  loop  ([f  (max-floor)]  [l  null])
	  (cond
	    [(= f 0)  l]
	    [(or (demand? f) (down-call? f) (up-call? f))
	     (loop  (sub1 f)  (cons f l))]
	    [else  (loop  (sub1 f)  l)]))))
	...
	(update-input)
	(let 
            ([ans  (Next-Floor  dir (current-floor)  
                                (list-of-all-calls)
                                (list-of-up-calls) 
                                (list-of-down-calls) 
                                (list-of-demands))]) 
    ...
    (define display-elevator-status 
      (lambda (d f all-calls up-calls down-calls demands)
        (display "--------------------")
        (newline)
        (display "Heading in direction ")
        (display d)
        (newline)
    ...
    (define  run/proc 
      (lambda (f)
	(check-proc 'run f 6 'first "6 arguments")
	(set! Next-Floor 
              (lambda params 
                (apply display-elevator-status params)
                (apply f params)))
	(start-program)))
