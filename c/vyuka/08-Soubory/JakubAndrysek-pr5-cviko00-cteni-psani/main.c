...
Tpole * nactiSoubor(FILE *f)
{
  ...
  Tpole* pole = novePole(pocetRadku);

  if(!pole) {
    printf("Pole nevytvoreno\n");
    return EXIT_FAILURE;
  }
  ...
  while(fscanf(f, "%f\n", &pole->prvek[pocitadlo]) == 1) {
//    printf("x %f\n", pole->prvek[pocitadlo]);
    pocitadlo++;
  }

  pole->delka = pocitadlo;
  // TODO Vrať ukazatel na strukturu s přečteným polem (proměnnou typu Tpole *).
  return pole;
}

void vytiskniNormalne(FILE *out, Tpole * p) {
  fprintf(out, "%d\n", p->delka);
